...
function resize(){
  ...
  const marginBot = playing
    ? Math.max(phone ? 200 : 180, Math.round(handH + (phone ? 12 : 24)))
    : 24;
...
class Hand3D {
  ...
  resize(){
    ...
    this.camera.fov = 34;
    /* Как на компе: тот же масштаб карт; Z чуть дальше только чтобы ряд влез по ширине */
    const phone = w <= 520;
    const baseZ = 420;
    const zFit = phone ? baseZ * Math.max(1, 390 / Math.max(320, w)) : baseZ;
    this.camera.position.set(0, 55, zFit);
    this.camera.lookAt(0, 8, 0);
  ...
  layout(){
    ...
    const w = this.canvas.clientWidth || window.innerWidth;
    const phone = w <= 520;
    /* На телефоне растягиваем ряд по ширине экрана, как веер на компе */
    const gap = phone
      ? Math.min(150, (w * 1.05) / Math.max(n, 1))
